...
// JS UDFs depend on the optional `isolated-vm` install. On a runner whose
// Node/platform has no prebuild and no working toolchain (e.g. Node 26
// pre-release builds, where the cppgc headers don't yet line up), the
// install is silently skipped and JS UDFs aren't available. Detect that
// up front and skip the whole suite with a clear reason — the rest of the
// emulator works without isolated-vm; CI for those runners shouldn't fail
...
const skipOpt = SKIP_REASON !== undefined ? { skip: SKIP_REASON } : {};
...
test('JS UDF: simple math (FLOAT64)', skipOpt, async () => {
  await query(`
...
test('JS UDF: NULL propagation (function body sees null)', skipOpt, async () => {
  await query(`
...
test('JS UDF: STRING in, STRING out', skipOpt, async () => {
  await query(`
...
test('JS UDF: INT64 surfaces as JS Number (BQ contract — lossy past 2^53)', skipOpt, async () => {
  await query(`
...
test('JS UDF: NUMERIC values surface as JS strings', skipOpt, async () => {
  await query(`
...
test('JS UDF: thrown error becomes a BQ error', skipOpt, async () => {
  await query(`
...
test('JS UDF: OR REPLACE re-registers the function', skipOpt, async () => {
  await query(`
...
test('JS UDF: compile error surfaces as BQ error', skipOpt, async () => {
...
test('Sandbox: UDF cannot access globalThis.process / require / Buffer', skipOpt, async () => {
  await query(`
...
test('Sandbox: infinite loop UDF hits the 5 s CPU timeout', skipOpt, async () => {
  await query(`
...
test('Sandbox: memory-hog UDF hits the 128 MB cap', skipOpt, async () => {
  await query(`
...
test('JS UDF: BOOL arg + return', skipOpt, async () => {
  await query(`
...
test('JS UDF: INT64 return from a string value parses into BigInt', skipOpt, async () => {
...
test(
  'JS UDF: OPTIONS(library=[...]) reports an error on HTTP non-200 response',
  skipOpt,
  async () => {
...
test('JS UDF: unsupported argument type rejects at CREATE FUNCTION', skipOpt, async () => {
  const body = await query(`
...
test('JS UDF: unsupported return type rejects at CREATE FUNCTION', skipOpt, async () => {
  const body = await query(`
...
test(
  'JS UDF: OPTIONS(library=[...]) reports an error when the URL is unreachable',
  skipOpt,
  async () => {
...
test(
  'JS UDF: OPTIONS(library=[...]) reports an error when the library exceeds the size cap',
  skipOpt,
  async () => {
    const http = await import('node:http');
...
test(
  'JS UDF: OPTIONS(library=[...]) reports an error when the library throws during load',
  skipOpt,
  async () => {
    const http = await import('node:http');
    const libServer = http.createServer((_req, res) => {
...
test(
  'JS UDF: OPTIONS(library=[...]) fetches and injects the library into the isolate',
  skipOpt,
  async () => {
